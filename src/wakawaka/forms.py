...
class DeleteWikiPageForm(forms.Form):
    ...
    def __init__(self, request, page, *args, **kwargs):
        ...
        if request.user.has_perm('wakawaka.delete_revision', page):
            self.base_fields['delete'].choices.append(('rev', _('Delete this revision')),)

        if request.user.has_perm('wakawaka.delete_revision', page) and \
           request.user.has_perm('wakawaka.delete_wikipage', page):
    ...
    def delete_wiki(self, request, page, rev):
        ...
        if self.cleaned_data.get('delete') == 'page' and \
           request.user.has_perm('wakawaka.delete_revision', page) and \
           request.user.has_perm('wakawaka.delete_wikipage', page):
        ...
        if self.cleaned_data.get('delete') == 'rev':
            ...
            if revision_length > 1 and request.user.has_perm('wakawaka.delete_revision', page):
            ...
            if revision_length <= 1 and \
               not request.user.has_perm('wakawaka.delete_wikipage', page):
